...
class TestBase(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.test_app.post('/add_task_sheet', data={
            "task_sheet_name": (
            os.path.dirname(os.path.abspath(__file__)) + '/test_samples/test.json', 'mock_name.json'
            )
        }, content_type='multipart/form-data')

    def extract_app_json(self):
        return json.loads(self.test_app.get('/get_task_sheet').data)
    ...
    def check_json(self, exp_json_name):
        ...
        with open("test_samples/" + exp_json_name) as exp_file:
            exp_json = json.load(exp_file)
            self.assertEqual(resp_json, exp_json)

    def add_tasks(self):
        self.test_app.post('/add_task', data={
            "task_complexity": 0,
    ...
    def move_tasks(self):
        for j in range(4, 7):
            for i in range(j, 7):  # look at tests
                self.test_app.post('/move_right/{}'.format(i))
